...
namespace ORB_SLAM3 {
    ...
    void MapDrawer::DrawMapPoints() {
        ...
        for (size_t i = 0, iend = vpMPs.size(); i < iend; i++) {
            ...
            pcl::PointXYZ pointAll;
            pointAll.x = pos(0);
            pointAll.y = pos(1);
            pointAll.z = pos(2);
            cloud_saved->points.push_back(pointAll);
        }
        glEnd();
        ...
        for (set<MapPoint *>::iterator sit = spRefMPs.begin(), send = spRefMPs.end(); sit != send; sit++) {
            ...
            pcl::PointXYZ pointRef;
            pointRef.x = pos(0);
            pointRef.y = pos(1);
            pointRef.z = pos(2);
            cloud_saved->points.push_back(pointRef);
        }
        if (cloud_saved->points.size()) {
            std::string timeNow = date::format("%F", std::chrono::system_clock::now());
            std::string pclName = timeNow + "-map.pcd";
            pcl::io::savePCDFileBinary(pclName, *cloud_saved);
        }
